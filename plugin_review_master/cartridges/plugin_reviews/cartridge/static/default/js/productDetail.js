...
base.updateAvailability = function () {
    $('body').on('product:updateAvailability', function (e, response) {
        ...
        if (response.product.reviews) {
            var reviews = '';
            var reviewerName = '';
            var heading = '<div class="col-sm-4 pull-right">'
                + '<input type="hidden" class="sort-productid" value="' + response.product.id + '">'
                + '<select class="form-control custom-select product-reviews-sorting">'
                + '<option value="" selected disabled>Sort By</option>'
                + '<option value="' + response.product.actionUrl + '" data-sort="lowestrated">Lowest Rated</option>'
                + '<option value="' + response.product.actionUrl + '" data-sort="highestrated">Highest Rated</option>'
                + '</select>'
                + '</div>'
                + '<div class =""><h2>Reviewed by ' + response.product.reviews.length + ' customers</h2></div>'
                + '<div class="col"><hr class="line-item-divider"></div>';

            response.product.reviews.reverse().forEach(function (review) {
                if (response.product.ratings !== null) {
                    var totalRatings = [1, 2, 3, 4, 5];
                    reviewerName = '<div class="row"><div class="col-6 reviewer-name">'
                ...
                reviews += reviewerName + '<div class="col-6">' + ratingHtml + '</div><div class="col-6 reviewText">' + review.review + '</div>'
                    + '<div class="col-6 reviewedDate">Submitted ' + review.date + '</div></div>'
                    + '<div class="col"><hr class="line-item-divider"></div>';
            });
            ...
            if (reviews) {
                ...
                response.$productContainer.find('.reviews .content').empty().html(reviews);
            }
        } else if (!response.$productContainer.find('.reviews').hasClass('hidden-xl-down')) {
            response.$productContainer.find('.reviews').addClass('hidden-xl-down');
        }
